...
class Event{
	...
	public function new_event($place, $event_name, $event_start, $event_end, $event_date, $type){
		

		$data = [
			[$place, $event_name, $event_start, $event_end, $event_date, $type],
		];
		$stmt = $this->conn->prepare("INSERT INTO events (Venue, event_name, Event_start, Event_end, Event_date, type_id) VALUES (?,?,?,?,?,?)");
		try {
			$this->conn->beginTransaction();
			foreach ($data as $row)
			{
	...
	public function update_event($place, $event_name,$event_start, $event_end, $event_date, $id, $etype){
		

		$sql = "UPDATE events SET Venue=:Venue, event_name=:event_name,Event_start=:Event_start, Event_end=:Event_end, Event_date=:Event_date, type_id=:type_id WHERE event_id=:event_id";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':Venue'=>$place,':event_name'=>$event_name, ':Event_start'=>$event_start,':Event_end'=>$event_end,':Event_date'=>$event_date,':type_id'=>$etype,':event_id'=>$id));
		return true;
	}
	...
	public function list_events_search($keyword){
		
		//$keyword = "%".$keyword."%";

		$q = $this->conn->prepare('SELECT * FROM `events` WHERE `event_name` LIKE ?');
