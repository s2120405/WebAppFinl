...
class Transac{
	...
	public function new_transac($cust_id, $item, $cust_qty){
		...
		$data = [
			[ $cust_id, $item, $cust_qty, $NOW,$NOW,],
		];
		$stmt = $this->conn->prepare("INSERT INTO rental_transac (cust_id, rental_id, cust_qty, date_add) VALUES (?,?,?,?)");
		try {
	...
	public function update_transac($cust_id, $item, $cust_qty, $Ttype){
		

		$sql = "UPDATE rental_transac SET cust_id=:cust_id, cust_qty=:cust_qty, rental_id=:rental_id WHERE trans_id=:trans_id";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':cust_id'=>$cust_id,':cust_qty'=>$cust_qty,':rental_id'=>$item,':trans_id'=>$Ttype));
		return true;
	}
